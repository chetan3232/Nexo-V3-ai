...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const isIde = location.pathname === '/demo' || location.pathname === '/ide' || location.pathname === '/chat';
        ...
        <header className="fixed top-0 w-full z-40 transition-all duration-300 bg-[#fbf9f6]/80 backdrop-blur-xl border-b border-stone-200/50">
          <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-2 text-sm font-medium bg-white/50 p-1.5 rounded-full border border-stone-200/50 backdrop-blur-md shadow-sm">
              ...
              <Link to="/ide" className={`px-4 py-1.5 rounded-full transition-all ${isActive('/ide')}`}>IDE</Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to="/ide" className="hidden sm:flex items-center gap-2 bg-stone-900 text-white px-5 py-2 rounded-full text-sm font-semibold hover:bg-stone-800 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
          ...
          <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12">
            <div className="space-y-4">
              ...
              <p className="text-stone-500 text-sm leading-relaxed">
                Modular AI-first IDE for multi-agent development workflows.
              </p>
            </div>
            <div>
              <h4 className="font-bold mb-6 text-stone-900">Project</h4>
              <ul className="space-y-3 text-sm text-stone-500 font-medium">
                <li><Link to="/ide" className="hover:text-stone-900 transition-colors">IDE Workspace</Link></li>
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-stone-500 text-sm bg-stone-50 p-3 rounded-xl border border-stone-100">
                ...
                <span className="font-medium">Multi-Model AI Runtime</span>
              </div>
            </div>
          </div>
        </footer>
