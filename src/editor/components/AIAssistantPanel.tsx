...
export function AIAssistantPanel({ onClose }: Props) {
  ...
  // Sync workspace-based conversations whenever workspace path changes
  useEffect(() => {
                  ...
                  {messages.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div style={{ fontSize: '13px', lineHeight: '1.6', color: '#c9d1d9', background: isUser ? 'rgba(255,255,255,0.04)' : 'transparent', border: isUser ? '1px solid #1f2937' : 'none', borderRadius: '8px', padding: isUser ? '7px 10px' : '0', minWidth: 0, overflow: 'hidden' }}>
                            <ReactMarkdown remarkPlugins={[remarkGfm]}>{msg.content || (isStreaming ? ' ' : '')}</ReactMarkdown>
                          </div>
                          
                          {/* Hover Fork action for messages */}
                          {!isUser && msg.id && (
                            <div 
                            ...
                            >
                              <button 
                                onClick={() => {
                                  const title = prompt('Enter a name for this new conversation branch:', `${msg.model?.split('/').pop() || 'Forked'} Branch`);
                                  if (title !== null) {
                                    void forkConversation(msg.id, title);
                                  }
                                }}
                              ...
                              >
                                <GitFork size={10} />
                                <span style={{ fontSize: '9px', fontWeight: 600 }}>Branch</span>
                              </button>
                            </div>
                          )}
                        </div>
