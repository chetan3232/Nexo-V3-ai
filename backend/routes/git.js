...
async function scanFilesForSecrets(filePaths) {
  const leaks = [];
...
router.post('/commit', async (req, res) => {
  ...
  // 1. Stage changes first so we can diff staged files
  await runGit('git add .');

  // 2. Scan staged files for secrets unless force is true
  if (force !== true) {
    const stagedResult = await runGit('git diff --cached --name-only');
    const stagedFiles = stagedResult.stdout ? stagedResult.stdout.split('\n').map(f => f.trim()).filter(Boolean) : [];
    const leaks = await scanFilesForSecrets(stagedFiles);
